...
def save_lead(data: dict):
    conn = db()
    conn.execute("INSERT INTO leads(parent_name,child_age,phone,interest,telegram_username,created_at) VALUES(?,?,?,?,?,?)", (data["parent_name"], data["child_age"], data["phone"], data["interest"], data.get("username", ""), datetime.now().isoformat(timespec="seconds")))
    conn.commit()
    conn.close()
...
def main_menu():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="🏫 О школе"), KeyboardButton(text="📚 Программа")], [KeyboardButton(text="🧠 Проверить математику"), KeyboardButton(text="🕘 Расписание")], [KeyboardButton(text="💰 Стоимость"), KeyboardButton(text="📍 Как нас найти")], [KeyboardButton(text="🎓 Записаться"), KeyboardButton(text="❓ Задать вопрос")]], resize_keyboard=True)
...
RESULTS = [(9, 10, "🏆 Математический гений!", "Видно, что с логикой и базовыми вычислениями всё отлично. Можно переходить к задачам повышенной сложности и олимпиадным заданиям."), (7, 8, "🌟 Уверенный ученик!", "Отличный результат. База в целом крепкая, но несколько тем стоит потренировать, чтобы знания стали устойчивыми."), (5, 6, "📖 Нужна поддержка", "Ребёнок старался. Результат показывает несколько зон, где спокойная регулярная практика поможет почувствовать себя увереннее."), (0, 4, "🔔 Нужна дополнительная помощь", "Тест показал заметные пробелы. Важно начать с понятного ребёнку уровня и постепенно восстановить базу без давления.")]


def quiz_keyboard(index: int):
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{chr(65 + i)}) {option}", callback_data=f"quiz:{index}:{i}")] for i, option in enumerate(QUESTIONS[index]["options"])])
...
async def show_question(message: Message, index: int):
    ...
    await message.answer(f"<b>Вопрос {index + 1} из 10</b> · {question['topic']}\n\n{question['text']}\n\nВыбери один вариант ответа 👇", reply_markup=quiz_keyboard(index))
...
async def quiz_answer(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        index, answer = int(index_raw), int(answer_raw)
    ...
    if answer == question["answer"]:
        ...
        await state.update_data(score=int(data.get("score", 0)) + 1, topic_scores=topic_scores, attempts=0)
        await callback.message.edit_text("✅ <b>Правильно!</b> Отлично, идём дальше.")
    else:
        attempts = int(data.get("attempts", 0)) + 1
        if attempts < 2:
            await state.update_data(attempts=attempts)
            await callback.message.edit_text(f"🤔 Пока не угадал. Попробуй ещё раз!\n\n💡 Подсказка: {question['hint']}", reply_markup=quiz_keyboard(index))
            return
        wrong = list(data.get("wrong_questions", []))
        wrong.append(index)
        await state.update_data(attempts=0, wrong_questions=wrong)
        await callback.message.edit_text(f"❌ В этот раз не получилось. Правильный ответ: <b>{chr(65 + question['answer'])}) {question['options'][question['answer']]}</b>\n\n💡 {question['hint']}")
    next_index = index + 1
...
async def finish_quiz(message: Message, state: FSMContext):
    ...
    lines.extend(["", f"<b>Что стоит потренировать:</b> {', '.join(weak_topics)}." if weak_topics else "🔥 Все пять тем пройдены без ошибок — отличный результат!", "", "Хотите получить персональные рекомендации и передать результат специалисту CoolClass?"])
    await state.set_state(QuizState.child_name)
    await message.answer("\n".join(lines), reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📋 Получить рекомендации", callback_data="quiz_lead")], [InlineKeyboardButton(text="🏫 Узнать о CoolClass", callback_data="quiz_school")]]))


@dp.callback_query(F.data == "quiz_school")
async def quiz_school(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer("<b>CoolClass — семейная школа во Внуково</b> 🏫\n\n• 7–9 детей в группе\n• математика каждый день\n• английский язык\n• понедельник–пятница, 09:00–18:00\n• отдельное здание и закрытая территория\n• питание включено\n\n📍 Москва, ул. Плотинная, 28\n📞 +7 929 692-92-08", reply_markup=main_menu())
...
async def complete_quiz_lead(message: Message, state: FSMContext, phone: str = ""):
    ...
    username = f"@{data['username']}" if data.get("username") else "нет username"
    await notify_admin("🧠 <b>Новая заявка из математической диагностики</b>\n\n" f"👧 Ребёнок: {data.get('child_name', 'не указано')}\n📊 Результат: {data.get('score', 0)}/10\n📚 Темы для практики: {topic_text}\n📞 Телефон: {phone or 'не указан'}\n💬 Telegram: {username}\n🕐 Время: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}")
    await state.clear()
