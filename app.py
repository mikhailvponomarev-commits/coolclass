...
TOKEN=os.getenv('BOT_TOKEN','').strip(); PORT=int(os.getenv('PORT','10000')); URL=os.getenv('RENDER_EXTERNAL_URL','').rstrip('/'); SECRET=os.getenv('WEBHOOK_SECRET',''); ADMIN=os.getenv('ADMIN_USERNAME','Mikhail7890').lstrip('@').lower(); DB=os.getenv('DB_PATH','coolclass.db')
...
async def begin(m,state:FSMContext): await state.set_state(Lead.parent); await m.answer('Оставьте заявку.\n\n<b>Как вас зовут?</b>',reply_markup=ReplyKeyboardRemove())
...
async def startup(*a,**kw):
 ...
 # No set_my_commands call: Telegram command names are ASCII-only.
