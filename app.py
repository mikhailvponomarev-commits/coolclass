...
async def on_startup(bot:Bot):
    db().close(); await bot.set_webhook(url=f'{PUBLIC_URL}{WEBHOOK_PATH}',secret_token=WEBHOOK_SECRET,drop_pending_updates=False); logger.info('CoolClass bot started with webhook')
async def on_shutdown(bot:Bot):
    try: await bot.delete_webhook(drop_pending_updates=False)
    except Exception: logger.exception('Failed to delete webhook')
...
def create_app():
    ...
    dp.startup.register(on_startup); dp.shutdown.register(on_shutdown); setup_application(app,dp,bot=bot); return app
