...
async def notify_admin(text):
    ...
    if not cid:
        log.warning('admin_chat_id is empty; owner must send /start to the bot')
        return
...
async def start(m: Message, state: FSMContext):
    await state.clear()
    if group(m):
        return await m.answer('👋 <b>Добро пожаловать в группу CoolClass!</b>\n\n/расписание — расписание\n/стоимость — стоимость\n/записаться — заявка\n/вопрос — вопрос\n\n🔒 Телефон и данные ребёнка собираются только в личном чате.')
    if (m.from_user.username or '').lower() == ADMIN:
        put('admin_chat_id', str(m.chat.id))
        return await m.answer('✅ Вы зарегистрированы. Новые заявки будут приходить сюда.', reply_markup=menu())
...
async def gt(m):
    ...
    return await {'расписание':schedule,'стоимость':price,'записаться':signup,'вопрос':question_info}[t](m)
...
async def health(request): return web.json_response({'status':'ok','bot':'CoolClass'})

async def run():
    conn().close()
    me=await bot.get_me(); log.info('BOT OK @%s id=%s', me.username, me.id)
    # Polling is intentionally used here: it avoids webhook/startup race conditions on Render.
    await bot.delete_webhook(drop_pending_updates=False)
    app=web.Application(); app.router.add_get('/', health); app.router.add_get('/health', health)
    runner=web.AppRunner(app); await runner.setup()
    site=web.TCPSite(runner, '0.0.0.0', PORT); await site.start()
    log.info('HTTP health server listening on %s', PORT)
    try:
