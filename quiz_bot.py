...
async def phone(message: Message, state: FSMContext):
    ...
    report = (
        "🔔 <b>Новая диагностика CoolClass</b>\n\n"
        f"👧 Ребёнок: {data.get('child_name','')}\n"
        f"📞 Телефон: {phone}\n"
        f"📊 Результат: <b>{data.get('score',0)}/10</b>\n\n"
        "🧩 <b>По темам:</b>\n" +
        "\n".join(f"{'🟢' if v == 2 else '🟡' if v == 1 else '🔴'} {k}: {v}/2" for k, v in data.get("topic_scores", {}).items()) +
        f"\n\n💬 Telegram: @{data.get('username') or 'нет'}"
    )
    if admin_id:
        try:
            await bot.send_message(admin_id, report)
            logger.info("Admin notification sent: chat_id=%s score=%s child=%s", admin_id, data.get("score", 0), data.get("child_name", ""))
        except Exception:
            logger.exception("Failed to send admin notification to chat_id=%s", admin_id)
    else:
        logger.warning("Admin chat is not registered. Ask @%s to send /start to this bot.", ADMIN_USERNAME)
    await state.clear()
